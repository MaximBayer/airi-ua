...
use app::{
  i18n::{TrayTranslations, get_locale_from_system},
  windows::{chat, onboarding, settings},
};
...
pub fn run() {
  ...
  tauri::Builder::default()
    ...
    .setup(|app| {
      ...
      // Get system locale and translations
      let locale = get_locale_from_system();
      let translations = TrayTranslations::new(&locale);

      let show_item = MenuItem::with_id(app, "show", &translations.show, true, None::<&str>)?;

      let quit_item = MenuItem::with_id(app, "quit", &translations.quit, true, None::<&str>)?;
      let settings_item = MenuItem::with_id(app, "settings", &translations.settings, true, None::<&str>)?;
      let hide_item = MenuItem::with_id(app, "hide", &translations.hide, true, None::<&str>)?;

      let position_center_item = MenuItem::with_id(app, "center", &translations.center, true, None::<&str>)?;
      let position_bottom_left_item =
        MenuItem::with_id(app, "bottom-left", &translations.bottom_left, true, None::<&str>)?;
      let position_bottom_right_item =
        MenuItem::with_id(app, "bottom-right", &translations.bottom_right, true, None::<&str>)?;
      ...
      let window_mode_fade_on_hover = MenuItem::with_id(app, "window-mode.fade-on-hover", &translations.fade_on_hover, true, None::<&str>)?;
      let window_mode_move = MenuItem::with_id(app, "window-mode.move", &translations.move_window, true, None::<&str>)?;
      let window_mode_resize = MenuItem::with_id(app, "window-mode.resize", &translations.resize, true, None::<&str>)?;

      let window_mode_sub_menu = Submenu::with_id_and_items(
        app,
        "window-mode",
        &translations.window_mode,
        true,
        &[
